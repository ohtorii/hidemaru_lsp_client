...
namespace HidemaruLspClient.ComContract
{
    sealed class Worker : IWorker
    {
        ...
        static Logger lspLogger_ = null;


        internal LanguageServerIdentifier key { get; }
        ...
        ServerCapabilities serverCapabilities_ = null;
        /// <summary>
        ...
        public Worker(LanguageServerIdentifier key)
        {
            this.key = key;
        }
        ...
        void InitializeClient()
        {
            ...
            client_ = new LanguageClient();
            client_.Start(
                new LanguageClient.LspParameter
                {
                    logger = lspLogger_,
        ...
        {
            readonly Diagnostic diagnostic_;
            readonly RangeImpl range_;
            public DiagnosticImpl(Diagnostic diagnostic)
            {
                diagnostic_ = diagnostic;
                range_ = new RangeImpl(diagnostic.range.start, diagnostic.range.end);
            }
            #region implement
            HidemaruLspClient_BackEndContract.IRange IDiagnostic.range => range_;
        ...
        sealed class LocationContainerImpl : ILocationContainer
        {
        ...
        ILocationContainer IWorker.Declaration(string absFilename, long line, long column)
        {
            ...
            return CommonProcessingOfGoto(absFilename, line, column, param, (arg) => client_.Send.TextDocumentDeclaration((DeclarationParams)arg));
        }
        ILocationContainer IWorker.Definition(string absFilename, long line, long column)
        {
            var param = new DefinitionParams();
            return CommonProcessingOfGoto(absFilename, line, column, param, (arg) => client_.Send.TextDocumentDefinition((DefinitionParams)arg));
        }
        ILocationContainer IWorker.TypeDefinition(string absFilename, long line, long column)
        {
            var param = new TypeDefinitionParams();

            return CommonProcessingOfGoto(absFilename, line, column, param, (arg) => client_.Send.TextDocumentTypeDefinition((TypeDefinitionParams)arg));
        }
        ILocationContainer IWorker.Implementation(string absFilename, long line, long column)
        {
            var param = new ImplementationParams();
            return CommonProcessingOfGoto(absFilename, line, column, param, (arg) => client_.Send.TextDocumentImplementation((ImplementationParams)arg));
        }
        ...
        ILocationContainer IWorker.References(string absFilename, long line, long column)
        {
            ...
            return CommonProcessingOfGoto(absFilename, line, column, param, (arg) => client_.Send.TextDocumentReferences((ReferenceParams)arg));
        }
        ...
        IServerCapabilities IWorker.ServerCapabilities
        {
            get
            {
                ...
                {
                    serverCapabilities_ = new ServerCapabilities(initializeResult_);
                }
        ...
        sealed class HoverImpl : IHover
        {
            public HoverImpl(Hover hover)
            {
                hover_ = hover;
            }
            public IMarkupContent contents => new MarkupContentImpl(hover_.contents);

            public HidemaruLspClient_BackEndContract.IRange range => new RangeImpl(hover_.range.start, hover_.range.end);

            Hover hover_;
        }
        sealed class MarkupContentImpl : IMarkupContent
        {
